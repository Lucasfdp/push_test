...
_pink()     { echo -ne "\033[35m$1"$(__nc); }

info(){
	tput cnorm
	echo -e "
$(_pink  "guilletester_ps Version 1.0 by gumoreno")


   * How to use it:
	
	$(_green "./guilletester_ps <how many ramdom numbers> <number of iterations>")	
			
		--> example:  ./guilletester_ps 500 30  -> 30 iterations of 500 ramdom numbers

	$(_green "./guilletester_ps --strict")						
			
		--> special selection of push_swap destroyer 500 numbers combinations :)  
		            
			(try "$(_green "./guilletester_ps --pepestrict") "if you can't waste time)

   * You can alse use the following flags:

	$(_green "--silent")  --> results will be shown at the end of the process

	$(_green "--log")	  --> over limit combinations will be saved in folder \"ps_log\"
...
print_to_log()
{
	echo -e $NB_OP "operations on "$1" ramdom numbers  ("$(date "+%c")")\nset ARG \""$ARG"\"; ./push_swap \$ARG | wc -l\n" >> ./ps_log/overlimit_log_fish.txt
	echo -e $NB_OP "operations on "$1" ramdom numbers  ("$(date "+%c")")\nARG=\""$ARG"\"; ./push_swap \$ARG | wc -l\n" >> ./ps_log/overlimit_log_bash.txt
}
...
print_results(){
	...
	if [[ $OVERLIMIT_NB -ge 1 && $log = true ]];then
		...
		echo -e "\r\t\t\t\t---> over limit combinations saved in folder \"ps_log\"\n"
	fi
	...
	if [[ $order_failed -ge 1 ]]; then
		echo -e "\n\r\033[1A"$(_red " ✘ ")" Order: failed "$order_failed"\033[1A"
...
do
	...
	NB_OP=$("./temp/push_swap" $ARG | wc -l)
	if [ $op_sys = "Darwin" ]; then
		ORDER=$("./temp/push_swap" $ARG | ./src/checker_Mac $ARG)
	elif [ $op_sys = "Linux" ]; then
		ORDER=$("./temp/push_swap" $ARG | ./src/checker_linux $ARG)
	fi
	if [ "$ORDER" = "KO" ]; then
